...
@Component({
  selector: 'app-home',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, UiInput, UiButton, UiRecentListings, TPipe],
  template: `
  ...
  styles: [`
    ...
    @media (max-width: 768px) {
      ...
      /* Ensure ui-input inside hero-input fills full width on mobile */
      .hero-input ui-input {
        width: 100%;
        display: block;
      }
      /* Make the host ui-button also stretch inside the flex column */
      .hero-input ui-input,
      ...
      /* In column direction the sections' auto margins shrink them to content
         width; force full width on mobile */
      .two-cols .section {
        width: 100%;
        margin: 0 0 24px;
      }
...
export class Home implements OnInit, OnDestroy {
  ...
  constructor(private router: Router) {
    // Re-fetch localized metadata (categories, waitlist) on language change
    effect(() => {
      this.i18n.lang();
      this.loadMetadata();
    });
  ...
  private loadMetadata() {
    this.metadataService.getMetadata().subscribe({
      ...
      error: (err) => {
        // Low-risk background data (categories/waitlist); log and don't block the rest of the home page.
  ...
  setSearchQueryFromKey(key: string) {
    ...
    // Don't search if the translation is missing (returns the key itself or empty)
  ...
  trackBySlug(index: number, cat: any): string {
    return cat.slug;
  }

  trackByTitle(index: number, wait: any): string {
    return wait.title;
  }
